...
class MainNavController: HBDNavigationController {
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if self.children.count > 0 {
            ...
            // 默认的返回按钮
            let backItem = UIBarButtonItem.init(image: R.image.nav_back(), style: .done, target: self, action: #selector(self.goback))
...
extension MainNavController
{
    ...
    override var preferredStatusBarStyle: UIStatusBarStyle {
        return self.presentedViewController?.preferredStatusBarStyle ?? self.topViewController?.preferredStatusBarStyle ?? .default
    }
